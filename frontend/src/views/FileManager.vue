<template>
  <v-container fluid>
    ...
    <!-- 에디터 -->
    <v-dialog v-model="showEditor" max-width="900" persistent>
      <v-card>
        <v-card-title class="d-flex align-center">
          <v-icon class="mr-2">mdi-file-document-edit</v-icon>{{ editingFile?.name }}
          <v-spacer /><v-btn icon="mdi-close" variant="text" @click="showEditor=false" />
        </v-card-title>
        <v-card-text>
          <v-textarea v-model="editorContent" rows="20" auto-grow variant="outlined"
            style="font-family:monospace;font-size:13px;line-height:1.4" />
        </v-card-text>
        <v-card-actions>
          <v-spacer /><v-btn @click="showEditor=false">취소</v-btn>
          <v-btn color="primary" :loading="saving" @click="saveFile">저장</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
