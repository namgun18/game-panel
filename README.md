...
2. OAuth2 → Redirect URI: `https://game.namgun.or.kr/api/auth/discord/callback`
3. Bot → 토큰 복사
...
| `APP_URL` | `https://game.namgun.or.kr` | 외부 접근 URL (OAuth 콜백 등에 사용) |
| `DEBUG` | `false` | 디버그 모드 |
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `SMTP_HOST` | `mail.namgun.or.kr` | SMTP 서버 |
...
│   ├── FileManager.vue # 웹 파일 탐색기
│   ├── GameRequest.vue # 게임 서버 신청
│   ├── AdminUsers.vue  # 사용자 관리
│   ├── AdminPermissions.vue # 권한 관리
│   ├── AdminInvites.vue # 초대 코드 관리
│   └── AdminRequests.vue # 게임 신청 관리
